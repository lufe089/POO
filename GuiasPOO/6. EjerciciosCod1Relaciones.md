...
3. Implementa el método `agregarAutomovil(Automovil * automovil)` en la clase `Persona`, el cual debe permitir asociar un automóvil a la persona. En este caso recibirá un apuntador a automovil
...
4. En la clase `Usuario`, implementa:
   - Un método `comprarLibro(Libro  * libro)`, que agregue un apuntador a libro a la colección del usuario.
   - Un método `regalarLibro(int idLibro)`, que busque el libro de la colección y lo elimine
5. En el `main()`, realiza las siguientes acciones:
   - Crea al menos dos apuntadores a objetos de la clase `Usuario` con diferentes nombres e identificadores.
   - Crea al menos tres apuntadores a objetos de la clase `Libro` con diferentes títulos, autores y géneros.
