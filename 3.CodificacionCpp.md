...
1. **Archivos de encabezado (`.h`)**: Contienen las **declaraciones** de clases, funciones y constantes. Normalmente incluyen:
...
Para escribir y ejecutar programas en C++ utilizaremos **Code::Blocks**, un entorno de desarrollo integrado (IDE). Un IDE es una herramienta que reúne en un mismo lugar el editor donde se escribe el código, el compilador que lo convierte en un programa ejecutable y algunas utilidades para organizar y depurar el proyecto.

Code::Blocks es una herramienta **gratuita y de código abierto** que se usa con frecuencia en cursos de programación porque es sencilla de instalar y permite empezar a trabajar rápidamente con C++.

**Code::Blocks no es el compilador**, sino el entorno que lo utiliza. Normalmente se instala junto con **GCC (MinGW)**, que es el compilador encargado de transformar el código en un programa que el computador puede ejecutar.

Cuando presionas **Build and Run**, Code::Blocks realiza tres pasos:

1. Compila el código usando el compilador.  
2. Si no hay errores, genera el archivo ejecutable.  
3. Ejecuta el programa y muestra el resultado en la consola.

En este curso utilizaremos Code::Blocks porque facilita la organización de proyectos con varios archivos y permite concentrarse en aprender los conceptos del lenguaje C++ y de la programación orientada a objetos.
